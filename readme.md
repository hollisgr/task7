...
*BenchmarkDefaultConcat-16 — Это название теста. Число 16 показывает, сколько логических ядер процессора использовал Go (твои потоки).
190896 (Итерации) — Столько раз выполнился цикл внутри b.Loop(). Чем выше это число, тем быстрее работает код.
5789 ns/op (Наносекунды) — Среднее время выполнения одного прохода цикла. У StringsBuilder оно в десятки раз меньше, чем у обычного сложения.
3320 B/op (Байты) — Сколько оперативной памяти (в куче) выделилось за одну операцию. У тебя StringsBuilder съел всего 3 КБ, а обычный плюс — 530 КБ. Разница в 160 раз!
9 allocs/op (Аллокации) — Сколько раз программа просила у системы новый кусок памяти. В плохом примере у тебя было 999 запросов, а в хорошем — всего 9.
Почему это критично:
Каждая «аллокация» нагружает Garbage Collector (сборщик мусора). Если их много (как 999), программа будет тормозить, пока Go пытается очистить память.*
